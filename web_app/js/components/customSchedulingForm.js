          ...
          <div class='form-group col-md-4'>
            <div class='card'>
              <ul class='list-group list-group-flush' style={listStyle} >
                {this.props.createdSchedules.map(value => {
                  return <li class='list-group-item'>{value.date}, {value.startHour} - {value.endHour}{value.repeats !== 'NoRepetition' ? `, ${value.repeats}` : ''}</li>
                })}
              </ul>
            </div>
          </div>
