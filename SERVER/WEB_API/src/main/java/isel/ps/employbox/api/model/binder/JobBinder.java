...
public class JobBinder implements ModelBinder<Job, OutJob, InJob, Long> {
    ...
    public List<OutJob> bindOutput(List<Job> list) {
        return list
            .stream()
            .map((curr)-> new OutJob(curr.getAccountID(), curr.getTitle(), experienceBinder.bindOutput(curr.getExperiences().get()), curr.getAddress(), curr.getWage(),
                    curr.getDescription(), curr.getSchedule(), curr.getOfferBeginDate(), curr.getOfferEndDate(), curr.getOfferType()))
    ...
    public List<Job> bindInput(List<InJob> list) {
        return list
                .stream()
                .map((curr)-> new Job(curr.getAccountID(), curr.getJobID(), curr.getTitle(), curr.getAddress(), curr.getWage(), curr.getDescription(), curr.getSchedule(),
                        curr.getOfferBeginDate(), curr.getOfferEndDate(), curr.getOfferType())
                )
                .collect(Collectors.toList());
    }
    ...
    public OutJob bindOutput(Job job) {
        return new OutJob(job.getAccountID(), job.getTitle(), experienceBinder.bindOutput(job.getExperiences().get()), job.getAddress(), job.getWage(), job.getDescription(),
    ...
    public Job bindInput(InJob curr) {
        return new Job(curr.getAccountID(), curr.getJobID(), curr.getTitle(), curr.getAddress(), curr.getWage(), curr.getDescription(), curr.getSchedule(),
                curr.getOfferBeginDate(), curr.getOfferEndDate(), curr.getOfferType()
        );
    }
}
